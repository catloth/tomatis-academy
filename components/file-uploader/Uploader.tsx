...
export function Uploader({ onChange, value, fileTypeAccepted }: iAppProps) {
  ...
  const uploadFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      if (!presignedResponse.ok) {
        toast.error("Não foi possível obter uma URL pré-assinada.");
      ...
      toast.error("Ocorreu uma falha ao enviar o arquivo.");
  ...
  const onDrop = useCallback(
    (acceptedFiles: File[]) => {
      if (acceptedFiles.length > 0) {
        ...
        // revokeObjectURL call to prevent memory leaks
  ...
  async function handleRemoveFile() {
    ...
    try {
      ...
      if (!response.ok) {
        toast.error("Ocorreu um erro ao tentar excluir o arquivo do servidor.");
